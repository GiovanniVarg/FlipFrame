import {publicPriceQuote,assumedOutputShape} from './pricing.mjs';
...
export function createOriginalService({db,save,adapter,mediaPath,mediaUrl,runMedia,download,importProject,budgetLimit,schedule,storageAvailable=()=>{},pause=()=>new Promise(r=>setTimeout(r,3000))}){
 ...
 async generate(id,owner){const d=own(id,owner);return exclusive(id,async()=>{if(['queued','running','completed'].includes(d.status))return safe(d);if(d.status!=='review')throw new Error('Review the cost first.');if(Object.values(db.originals).some(x=>x.status==='unknown')||Object.values(db.jobs||{}).some(x=>x.status==='unknown'))throw new Error('Reconcile the earlier uncertain provider request before starting another.');const fresh=await quote(d);if(fresh.estimatedUsd>d.quote.estimatedUsd){d.quote=fresh;save();throw new Error('The estimate increased. Review the new amount before generating.');}db.spend=reserve(db.spend,budgetLimit(),fresh.estimatedUsd);d.hold=budgetHold(fresh.estimatedUsd);d.quote=fresh;d.status='queued';save();schedule();return safe(d)})},
