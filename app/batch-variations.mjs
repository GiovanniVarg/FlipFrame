import {budgetHold,sumMicroUsd} from './budget.mjs';
...
export function createBatches({db,save,getProject,queue,budgetLimit,schedule=()=>{}}){
 ...
 const total=sumMicroUsd(plans.map(p=>p.route.estimatedUsd));const b={id:randomUUID(),baseRevisionId:p.activeRevisionId,plans,referenceImageId:ref?.id,total,hold:sumMicroUsd(plans.map(p=>budgetHold(p.route.estimatedUsd))),createdAt:new Date().toISOString(),jobIds:[]};const old=p.batches;p.batches={...Object.fromEntries(Object.entries(old||{}).filter(([,v])=>v.jobIds.length||!Number.isFinite(Date.parse(v.createdAt))||Date.now()-Date.parse(v.createdAt)<=15*60000)),[b.id]:b};try{save()}catch(e){p.batches=old;throw e}return snapshot(p,b);};
