...
const manualIntents=new Set([...Object.keys(EDITOR_WORKFLOWS),'mute','gain','replace_audio','replace_picture','picture','object','generate_audio']);
...
export function computeGenerationWindow(p,start,end,resolution='720p'){
 ...
 return {processStart,processEnd,resolution,pricingDimensions:{width,height,inputSeconds:duration,outputSeconds:duration},estimatedUsd};
}
export function buildEditPlan(p,input,decision){
 ...
 const temporalText=text.replace(/[-+−]?\d+(?:\.\d+)?\s*dB\b/gi,' ');
 let action=Object.hasOwn(labels,decision.action)?decision.action:'clarify';
 ...
 if(['picture','object','generate_audio'].includes(action)){
  ...
  plan.processStart=window.processStart;plan.processEnd=window.processEnd;plan.inputDuration=window.processEnd-window.processStart;
...
export function createConversationService({db,save,getProject,classify,execute,configured=()=>false}){
 ...
 async function message(projectId,owner,input){
  ...
  const fingerprint=createHash('sha256').update(JSON.stringify([input.text,input.baseRevisionId,input.start,input.end,input.quality,input.referenceImageId??'',input.reviewCandidateId??'',...(input.intent===undefined?[]:[input.intent])])).digest('hex');const existing=c.requests[input.idempotencyKey];
  ...
  const task=(async()=>{let plan;try{const previous=c.plans.at(-1);const decision=input.intent!==undefined?{action:input.intent,source:'manual'}:await classify({text:input.text,context:{selectedRange:{start:input.start??0,end:input.end??p.duration},hasCandidate:Object.values(db.candidates||{}).some(a=>a.projectId===projectId&&a.baseRevisionId===p.activeRevisionId),hasMask:false,previousAction:previous?.action,previousInstruction:previous?.instruction,availableCapabilities:Object.keys(labels)}});plan=buildEditPlan(p,input,decision);if(EDITOR_WORKFLOWS[plan.action]?.candidate){const candidate=db.candidates?.[input.reviewCandidateId];if(!candidate||candidate.projectId!==p.id||candidate.baseRevisionId!==p.activeRevisionId){plan.status='needs_input';plan.explanation='Open the candidate you want to work on, then send this request again. No candidate has been chosen for this action.';}else{plan.targetCandidateId=candidate.id;plan.targetCandidateLabel=candidate.label||'Selected candidate';}}}catch{plan=buildEditPlan(p,input,{action:'clarify',source:'clarify',reason:'I could not finish routing this request. No video generation was submitted. Choose the kind of change to prepare a plan.'});}
   if(referenceImage){plan.referenceImage=referenceImage;plan.warnings.push(['picture','object'].includes(plan.action)?'The attached reference will be sent to Higgsfield when you create this preview. Reference matching is not guaranteed.':'Reference attached; this action does not send it to a generation model.');}
