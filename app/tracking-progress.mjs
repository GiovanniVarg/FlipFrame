...
export function trackingProgress(line){
 ...
 try{const p=JSON.parse(line.slice(19));if(!['preparing','tracking'].includes(p.stage)||!Number.isInteger(p.completed)||!Number.isInteger(p.total)||p.total<0||p.completed<0||p.completed>p.total)return null;return {stage:p.stage,completed:p.completed,total:p.total};}catch{return null;}
}
