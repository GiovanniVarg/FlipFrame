...
test('authenticated app isolates project, media, jobs and allows restore after trash',async()=>{
 ...
 try{let ready=false;for(let i=0;i<80;i++){try{if((await fetch(base+'/api/health')).ok){ready=true;break}}catch{}await new Promise(r=>setTimeout(r,100));}assert.ok(ready,log);
  ...
  assert.equal((await call(p.sourceUrl,undefined,bob)).status,404);assert.equal((await call(p.thumbnailUrl,undefined,bob)).status,404);const sourceResponse=await call(p.sourceUrl,undefined,alice);assert.equal(sourceResponse.status,200);await sourceResponse.arrayBuffer();
  ...
  let finished;for(let i=0;i<600;i++){finished=await(await call('/api/jobs/'+job.id,undefined,alice)).json();if(['completed','failed'].includes(finished.status))break;await new Promise(r=>setTimeout(r,100));}assert.equal(finished.status,'completed',JSON.stringify(finished));assert.equal((await call(finished.result.url,undefined,bob)).status,404);
 ...
 }finally{const closed=new Promise(r=>child.once('close',r));child.kill();await closed;const resolved=path.resolve(dir);assert.ok(resolved.startsWith(path.resolve(os.tmpdir())+path.sep));await fs.promises.rm(resolved,{recursive:true,force:true,maxRetries:10,retryDelay:300});}
});
