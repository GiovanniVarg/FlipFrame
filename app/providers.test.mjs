...
test('exact authenticated description computes rounded-up reservation without asserting hard cap',async()=>{
 ...
 assert.equal(quote.billableVideoTokens,216000);assert.equal(quote.estimatedUsd,2.78);assert.equal(quote.maximumUsd,2.78);
...
test('exact text-to-video description prices zero input seconds and requested output duration',async()=>{
 ...
 assert.equal(quote.billableVideoTokens,108000);assert.equal(quote.estimatedUsd,2.32);assert.equal(quote.formulaVerified,true);assert.equal(quote.verified,false);assert.equal(quote.isHardCap,false);
